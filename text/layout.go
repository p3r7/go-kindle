...
func Draw(dst *image.RGBA, text string, face Face, options *DrawOptions) {
	var layoutOp LayoutOptions
	var drawOp DrawImageOptions
	...
	for _, g := range AppendGlyphs(nil, text, face, &layoutOp) {
		...
		pos := image.Pt(int(g.X), int(g.Y))
		bounds := g.Image.Bounds()
		rect := image.Rectangle{
			Min: pos,
			Max: pos.Add(bounds.Size()),
		}
		draw.Draw(dst, rect, g.Image, bounds.Min, draw.Over)
