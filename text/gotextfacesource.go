...
type GoTextFaceSource struct {
	...
	glyphImageCache map[float64]*cache[goTextGlyphImageCacheKey, *image.RGBA]
...
func (g *GoTextFaceSource) getOrCreateGlyphImage(goTextFace *GoTextFace, key goTextGlyphImageCacheKey, create func() (*image.RGBA, bool)) *image.RGBA {
	if g.glyphImageCache == nil {
		g.glyphImageCache = map[float64]*cache[goTextGlyphImageCacheKey, *image.RGBA]{}
	}
	if _, ok := g.glyphImageCache[goTextFace.Size]; !ok {
		g.glyphImageCache[goTextFace.Size] = newCache[goTextGlyphImageCacheKey, *image.RGBA](128 * glyphVariationCount(goTextFace))
	}
